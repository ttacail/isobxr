...
designed to perform and explore stable isotope box modelling of open or closed systems.
...
in both static (*e.g.*, at steady state) or dynamic modes (*e.g.*, in reaction to a perturbation), 
build complex scenarios, and sweep the space of parameters in both static and dynamic modes.
...
# FALSE if no pandoc/Rstudio 
```

The user is also invited to download the [reference templates](https://ttacail.github.io/source/demos/0_excel_templates.zip) 
for all master files used as entries to the functions and described in this vignette

The user can also download the [demonstration master files](https://ttacail.github.io/source/demos/1_ABC_tutorial.zip) 
used in the [Getting started 1](https://ttacail.github.io/isobxr/articles/Demo_ABCD.html) and 
[Getting started 2](https://ttacail.github.io/isobxr/articles/Demo_ABCD_composite.html) demonstrations.

The [isobxr](https://ttacail.github.io/isobxr/) package is working under
R 3.5.2 to 4.0.3 versions.
...
You can load the [isobxr](https://ttacail.github.io/isobxr/) package as follows:
```{r eval = FALSE}
library(isobxr)
```
...
----
# Theory of box modelling and formalism

The theory and formalism shown in this section is derived from the chapter 7
(*Dynamic systems*) of
...
## Principles of stable isotopes box modelling
...
The `run_isobxr` function allows to run the [isobxr](https://ttacail.github.io/isobxr/) 
stable isotope box model, assess and render the design of the model 
and automatically run `num_slvr` or `ana_slvr` depending on the model design.

As defined in the documentation, `run_isobxr`

1. takes a series of inputs from a working directory as well as from the user in the console

1. stores a track of all the runs (names, conditions, characteristics) 
both in run specific files and in a global LOG **csv** file

1. returns several outputs files to automatically created and managed directories and subdirectories
...
1. The `run_isobxr` function first extracts the configuration of the system 
(boxes, fluxes, fractionation coefficients) and of the given run (run duration, initial conditions):   
    i) These parameters come in majority from the **isobxr master file** 
      (isotope system details, boxes details, flux and fractionation coefficient lists)
    i) Run specific parameters are directly set as parameters of the function 
      (run duration, run resolution, time units for the plot)
    i) Optional parameters allow the manual forcing of some parameters 
      (FORCING_RAYLEIGH, FORCING_SIZE, FORCING_DELTA, FORCING_ALPHA)
    
1. The `run_isobxr` function then evaluates the general design of the box model.  
    i) It ignores boxes possibly named by user but isolated from the general system
      (no inward nor outward fluxes)
    i) It determines if the system is closed or open by identifying the presence/absence 
    of boxes with the **INFINITE** status  
        + In the case of a closed system, *i.e.* with no declared **INFINITE** boxes, 
        it displays the following message in the console:  
            - **< All boxes are FINITE >**  
        + In the case of an open system, *i.e.* with declared **INFINITE** boxes, 
        it displays the list of infinite boxes in the console:  
            - **< The INFINITE boxes are ... BOX_i ... >**  
    i) It determines whether some finite boxes are out of balance (unbalanced inward/outward fluxes).
        + In the case of a globally balanced system, *i.e.* no out of balance finite boxes, 
        no message is printed in the console.
        + In the case of a system with at least one unbalanced finite box,
        it displays a message for each unbalanced finite box in the console:   
            - **< BOX_i IN/OUT is pos >**  
            if the balance of box *i* is positive (accumulation of X)  
            - **< BOX_i IN/OUT is neg (max run: empty_duration t units) >** 
            if the balance of box *i* is negative (loss of X).
                It also displays the maximum duration of the run until the box is totally emptied.
...
The first purpose of this column is to specify to the code whether the model represents an **open** or **closed system**.  
    ...
    this column is used by the [isobxr](https://ttacail.github.io/isobxr/) functions to set a color code in the automatic box model diagram edition.
